    ...
    Authorizable
{
    ...
    TieredSupply internal _tieredSupply =
        TieredSupply({
    ...
    function mint(string memory rarity_) external whenNotPaused nonReentrant {
    ...
    function mintWhitelist(string memory rarity_)
        external
        whenNotPaused
        nonReentrant
    {
    ...
    function _checkRarityBeforeMint(string memory rarity_) internal pure {
        require(
            keccak256(bytes(rarity_)) == keccak256("common") ||
                ...
                keccak256(bytes(rarity_)) == keccak256("slegend"),
            "PlanetHorseNFT: invalid argument"
        );
    }

    function _mintByRarity(address minter_, string memory rarity_)
        internal
        whenNotPaused
    {
        if (keccak256(bytes(rarity_)) == keccak256("common")) {
            ...
            require(
                tierSupply < _tieredSupply.COMMON_MAX_SUPPLY,
                "PlanetHorseNFT: Tier Limit Reached!"
            );
            require(
                paymentCurrency.transferFrom(minter_, address(this), 5000000),
                "PlanetHorseNFT: Payment Failed!"
            );
            ...
            require(
                tierSupply < _tieredSupply.RARE_MAX_SUPPLY,
                "PlanetHorseNFT: Tier Limit Reached!"
            );
            require(
                paymentCurrency.transferFrom(minter_, address(this), 7500000),
                "PlanetHorseNFT: Payment Failed!"
            );
            ...
            require(
                tierSupply < _tieredSupply.SRARE_MAX_SUPPLY,
                "PlanetHorseNFT: Tier Limit Reached!"
            );
            require(
                paymentCurrency.transferFrom(minter_, address(this), 12000000),
                "PlanetHorseNFT: Payment Failed!"
            );
            ...
            require(
                tierSupply < _tieredSupply.EPIC_MAX_SUPPLY,
                "PlanetHorseNFT: Tier Limit Reached!"
            );
            require(
                paymentCurrency.transferFrom(minter_, address(this), 15000000),
                "PlanetHorseNFT: Payment Failed!"
            );
            ...
            require(
                tierSupply < _tieredSupply.LEGEND_MAX_SUPPLY,
                "PlanetHorseNFT: Tier Limit Reached!"
            );
            require(
                paymentCurrency.transferFrom(minter_, address(this), 30000000),
                "PlanetHorseNFT: Payment Failed!"
            );
            ...
            require(
                tierSupply < _tieredSupply.SLEGEND_MAX_SUPPLY,
                "PlanetHorseNFT: Tier Limit Reached!"
            );
            require(
                paymentCurrency.transferFrom(minter_, address(this), 50000000),
                "PlanetHorseNFT: Payment Failed!"
            );
        ...
        returns (string memory)
    {
        ...
        if (_revealed[tokenId]) {
            return string(abi.encodePacked(_revealedTokenURIs[tokenId]));
        } else {
            return
                string(
                    abi.encodePacked(
                        _hiddenBaseURI,
                        string(_tokenTiers[tokenId]),
                        ".json"
                    )
                );
        }
    ...
    function setHiddenURI(string memory baseURI) external onlyOwner {
        ...
        onlyAuthorized
    {
        if (authorized[msg.sender]) {
            require(
                !_revealRequestCompleted[tokenId],
                "PlanetHorseNFT: Duplicated reveal request"
            );
        }
    ...
    function reveal(uint256 tokenId) external whenNotPaused {
        ...
        require(
            ...
            "PlanetHorseNFT: only NFT owner can unbox"
        );
    ...
    function registerWhitelist(address[] memory addrs) external onlyOwner {
        for (uint256 i = 0; i < addrs.length; i++) _whitelist[addrs[i]] = true;
    }
    ...
    function setWhitelistSaleTime(uint256 whitelistSaleTime_)
        external
        onlyOwner
    {
        _whitelistSaleTime = whitelistSaleTime_;
    ...
    function setunlimitedSaleTime(uint256 unlimitedSaleTime_)
        external
        onlyOwner
    {
        _unlimitedSaleTime = unlimitedSaleTime_;
